#!/usr/bin/env python3
"""Rebuild the ksd-liquidation-live artifact HTML by embedding the latest
TOTAL LIQUIDATION figures as a JS PAYLOAD constant.

Total liquidation = KSBC tertiary shop sales + CFD invoice sales + BAR invoice
sales (all three legs, in cases), per Abhay's definition (CLAUDE.md).

The artifact iframe can't call workspace.bash (sandboxed), so the data is baked
into the HTML at refresh time, exactly like the warehouse-live artifact. The
liquidation-live scheduled task runs this script every evening, then calls
mcp__cowork__update_artifact with the path printed on the last stdout line.

Auto-detects the CURRENT working month (newest analysis workbook by mtime) so
the dashboard auto-rolls month to month. Headline totals come from the
AUTHORITATIVE roll-up sheets (KSBC '... COMBINED', Secondary 'COMBINED
DISPATCHES') via the shared build_total_liquidation extractors — same source of
truth as TOTAL LIQUIDATION -<MONTH>.xlsx. The daily trend is computed from the
KSBC daily raw sheets (Shop Out) + Secondary dispatches by Inv/GTN date; it is
the SHAPE of liquidation over the month and may differ from the COMBINED
headline by a fraction of a percent (KSBC case-conversion rounding).

Single source of truth for the artifact HTML is
.claude/scripts/liquidation_live_template.html — edit there for layout changes.

Usage:
    python3 refresh_liquidation_live_artifact.py [--month JUNE] [--out PATH]

Prints a multi-line summary, then a final line:
...
# Reuse the locked extraction logic so headline numbers match the workbook.
sys.path.insert(0, str(Path(__file__).resolve().parent))
import build_total_liquidation as B  # noqa: E402
...
def claude_root() -> Path:
    ...
    return here.parent.parent.parent  # .../<Claude>/.claude/scripts/<this file>


def template_path() -> Path:
    return Path(__file__).resolve().parent / "liquidation_live_template.html"
...
def default_out_path() -> Path:
    ...
    if home:
        ...
        if outputs.is_dir():
            return outputs / "liquidation_live.html"
    return Path(__file__).resolve().parent / "_liquidation_live_refreshed.html"
...
def detect_current_month(root: Path) -> str:
    ...
    for f in folder.glob("*ANALYSIS.xlsx"):
        ...
        if best is None or mt > best[0]:
            best = (mt, mon)
    if best is None:
        raise SystemExit("No KSBC analysis workbook found to detect the month.")
    return best[1]
...
def build_payload(root: Path, month=None):
    ...
    if rows is None:
        raise SystemExit(f"Missing analysis source for {cur}: "
...
def build_bundle(root: Path, months=None, current=None, use_cache=True):
    ...
    if not data:
        raise SystemExit("No month could be built.")
    if cur not in data:
...
def write_artifact(payload, out_path: Path) -> Path:
    ...
    rendered = html.replace("{{PAYLOAD}}", json.dumps(payload, separators=(",", ":")))
...
def main() -> int:
    ...
    ap.add_argument("--month", default=None, help="Force the month the artifact opens on (default: auto-detect newest).")
    ap.add_argument("--months", default="all",
                    help="Months to ship in the filter: 'all' (default), 'current', "
                         "or a comma list e.g. JUNE,JULY,AUGUST.")
    ap.add_argument("--no-cache", action="store_true",
                    help="Rebuild every month from the workbooks, ignoring the payload cache.")
    ap.add_argument("--out", type=Path, default=None)
    args = ap.parse_args()

    root = claude_root()
    sel = args.months.strip().lower()
    if sel == "current":
        months = [(args.month or detect_current_month(root)).strip().upper()]
    elif sel == "all":
        months = None
    else:
        months = [m.strip().upper() for m in args.months.split(",") if m.strip()]
        bad = [m for m in months if m not in MONTHS_UP]
        if bad:
            raise SystemExit(f"Unknown month(s): {', '.join(bad)}")

    print("Months:")
    bundle = build_bundle(root, months, args.month, use_cache=not args.no_cache)
    out_path = args.out or default_out_path()
    written = write_artifact(bundle, out_path)

    p = bundle["data"][bundle["current"]]
    print(f"Filter:         {len(bundle['months'])} month(s) — "
          f"{', '.join(m['label'] for m in bundle['months'])}")
    print(f"Opens on:       {bundle['current']}")
    print(f"Month:          {p['month']} {p['year']}  ({p['periodLabel']})")
    print(f"As of:          {p['asOf']}  (day {p['daysElapsed']} of {p['daysInMonth']})")
    print(f"GRAND TOTAL:    {p['grand']:,.2f} cs")
    if p.get("shopStock"):
        _ss = p["shopStock"]
        _n = _ss["net"]
        print(f"SHOP STOCK:     {_n['closing']:,.2f} cs held in {_n['shops']} shops "
              f"across {len(_ss['bondOrder'])} bonds")
        print(f"  not moving:   {_n['stag']:,.2f} cs on {_n['stagLines']:,} "
              f"shop-SKU lines with zero sales "
              f"({_n['stag']/_n['closing']*100:.1f}% of stock held)")
    print(f"  KSBC:         {p['byType'].get('KSBC',0):,.2f} cs")
    print(f"  CFD:          {p['byType'].get('CFD',0):,.2f} cs")
    print(f"  BAR:          {p['byType'].get('BAR',0):,.2f} cs")
    print(f"Run-rate:       {p['runRate']:,.2f} cs/day  ->  projected {p['projected']:,.0f} cs")
    print(f"Outlets liq.:   {p['activeOutlets']} of {p['totalOutlets']}")
    print(f"Daily points:   {len(p['daily'])}")
    print(f"Top brand:      {p['brandSorted'][0][0]} ({p['brandSorted'][0][1]:,.0f} cs)")
    print(f"Top bond:       {p['bondSorted'][0][0]} ({p['bondSorted'][0][1]:,.0f} cs)")
    if p['prior']:
        print(f"Prior month:    {p['prior']['month']} {p['prior']['grand']:,.0f} cs "
              f"(run-rate {p['prior']['runRate']:,.1f} cs/day)")
